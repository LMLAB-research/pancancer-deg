#### Standalone batch analysis: every TCGA cancer versus OIS ####

# This script is completely standalone. It does not source or call another
# analysis script. Every cancer receives a separate result folder.
...
cancer_input_dir <- "/Users/danabiruk/Downloads/drive-download-20260812T194029Z-1-001"
...
batch_output_dir <- file.path(
  "/Users/danabiruk/Documents",
  "all_TCGA_OIS_overlaps"
)
...
#### 5. Analyze one cancer and save its separate result set ####
...
analyze_one_cancer <- function(cancer_file) {
  ...
  up_up <- intersect(cancer_up, ois_up)
  up_down <- intersect(cancer_up, ois_down)
  down_up <- intersect(cancer_down, ois_up)
  down_down <- intersect(cancer_down, ois_down)
  ...
  write.csv(
    ...
    file.path(cancer_output_dir, "OIS_duplicates_for_review.csv"),
    row.names = FALSE
  )
...
#### 6. Run every cancer in filename order ####
  ...
  run_status[[i]] <- tryCatch(
    ...
    error = function(error) {
      ...
      data.frame(
        ...
        input_file = cancer_file,
        output_dir = file.path(batch_output_dir, cancer_name),
        status = "FAILED",
...
#### 7. Save a technical run log ####

run_status <- do.call(rbind, run_status)

write.csv(
  run_status,
  file.path(batch_output_dir, "batch_run_status.csv"),
  row.names = FALSE
)
...
cat("\nSeparate result folders saved in:\n", batch_output_dir, "\n")
